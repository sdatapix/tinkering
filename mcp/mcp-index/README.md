...
```json
  ...
  "inputSchema": {
    ...
    "properties": {
      ...
      "ssn": {
        "type": "string",
        "description": "Social Security Number for US applicants. Used for eCBSV (SSA) SSN verification."
      },
      "email": {
        "type": "string",
        "format": "email",
...
Following is an example of a `remote` server.json generated by Claude with the help of uploaded OpenAPI document for an identity verification service.

```json
{
  "$schema": "https://static.modelcontextprotocol.io/schemas/2025-12-11/server.schema.json",
  "name": "io.transunion/truvalidate-identity-verification-mcp",
  "title": "TransUnion TruValidate Identity Verification",
  "description": "Verify identities via risk signals, watchlist screening, device risk, and eCBSV SSN validation.",
  "version": "1.0.0",
  "websiteUrl": "https://www.transunion.com/solution/truvalidate",
  "remotes": [
    {
      "type": "streamable-http",
      "url": "https://api.transunion.com/truvalidate/identity-verification/v1/mcp",
      "variables": {
        "region": {
  ...
  "_meta": {
    "io.modelcontextprotocol.registry/publisher-provided": {
      "upstreamApi": "https://api.transunion.com/truvalidate/identity-verification/v1",
      "upstreamApiSpec": "transunion-truvalidate.yml",
      "dataResidency": [
        "US"
      ],
      "complianceNotes": "Contains PII and SSN data. Ensure GLBA, FCRA, and applicable state privacy law compliance before use."
    }
  }
}
```
...
Following is an SEP-1649 compliant server card (to be updated once SEP 2127 is merged) hypothetically available from `https://www.transunion.com/solution/truvalidate/.well-known/mcp/server-card.json`. The server card is generated by Claude with the help of uploaded OpenAPI document.
...
```json
{
  ...
  "name": "io.transunion/truvalidate-identity-verification-mcp",
  "title": "TransUnion TruValidate Identity Verification",
  "description": "Verify applicant identities by screening their information against multiple data sources and aggregated risk signals. Access over 65 actionable data attributes and risk scores — including address verification, watchlist screening, device risk assessment, and eCBSV (SSA) SSN validation — via a single API call.",
  "version": "1.0.0",
  ...
  "remotes": [
    {
      "type": "streamable-http",
      "url": "https://api.transunion.com/truvalidate/identity-verification/v1",
    ...
    {
      "type": "sse",
      "url": "https://api.transunion.com/truvalidate/identity-verification/v1/sse",
      "supportedProtocolVersions": [ "2025-11-25" ],
      "authentication": {
        "required": true,
        "schemes": ["bearer", "oauth2"]
  ...
  "prompts": [],
  "_meta": {
    "publishedAt": "/.well-known/mcp/server-card.json",
    "mcpResource": "mcp://server-card.json"
  }
}
```
...
Typical enterprise AI architecture would be as following where enterprises would use an intermediary like an agent gateway (e.g. [agentgateway](https://agentgateway.dev/)) for the following and other reasons.

* Security and access control: provides RBAC, JWT authentication, TLS, and access policies purpose-built for agent workloads
...
* Observability: provides full OTel-based distributed tracing (spans for initialize, list_tools, call_tool), metrics, and structured access logs — all correlated by trace ID, with dynamic span enrichment like jwt.sub, mcp.tool, and request.path.
...
flowchart LR
    ...
    subgraph MCPGateway["🔀 Agent Gateway"]
        ...
        MCPRouter --OTel events (trace, log)--> MCPObs[OTel Telemetry]
    end
    LLM[🧠 LLM]<-->LLMR
    MCPSUpstream --Data flows-->Server["⚙️ MCP Server <br>example-mcp.com"]
    MCPObs --OTel events--> Observability["📊 Observability"]
...
```
In order to find sensitive data flow in communication between the MCP client and MCP server with or without an intermediary like an Agent Gateway without peeking into the payload, ChannelSeal could rely on telemetry data and correlate the MCP traffic with its MCP Server Index. Following sections describe how we would build the index and use it for detecting sensitive data.
...
flowchart LR
    ...
    A[AI Application <br>MCP Client]-.tool/ resource calls.->AG[Agent Gateway]-.tool/ resource calls.->MS["MCP Server <br>(remote)"]
    A--tool/ resource calls-->MS
    MS--API-->S[Service]
    A--OTel events-->C
    AG-.OTel events.->C[OTel Collector<br> + HTTP exporter]
    C--OTel events-->CC[ChannelSeal]
```
...
* Storing sensitive data elements, tools and resources for the MCP server in the MCP Server Index
* Continuous updating of the MCP Server Index by repeating the above steps

#### Indexing MCP Server

MSI would index the following attributes from MCP Server metadata in order to identify service provider.
...
* `description`
* `websiteUrl`
* `remotes.url` for `streameable-http` and `sse`
* `_meta.io.modelcontextprotocol.registry/publisher-provided`


#### Indexing MCP Tool

MSI would index the following attributes from an MCP tool in order to identify channel and sensitive data elements.
...
### Building The ChannelSeal MCP Server Index 
...
sequenceDiagram
    ...
    participant CMC as ChannelSeal <br>MCP Server Catalog
    ...
    participant CMSP as ChannelSeal <br>MCP Server Providers
